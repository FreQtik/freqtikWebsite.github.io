...
<article class="ia-mode-card"><b>Frequency structure</b><h3>Spectral &middot; Spectral BandSwap &middot; Spectral Zig Zag</h3><p>Fuse spectral character, divide the frequency range between sources, or create alternating spectral relationships.</p></article>
...
<section class="ia-section" id="ia-draw-path"><div class="ia-shell"><div class="ia-section-head"><div class="ia-copy"><span class="ia-kicker">Draw movement into the IR</span><h2>Paint the blend.<br/>Route source-time.</h2><p>Draw lets you author how A and B change across the response. Path goes further: the line can travel forward or backward through source-time while also moving between A and B.</p></div></div><div class="ia-mode-stage"><article class="ia-mode-intro"><div><span class="ia-eyebrow"><span class="ia-pulse"></span> Draw &middot; Free Draw &middot; Path</span><h3>The curve display becomes part of the instrument.</h3><p>B sits at the top and A at the bottom. The subtle A/B waveforms behind the graph act as landmarks, so transients, gaps, tails and later events can be targeted by eye instead of drawing blind.</p></div><div class="ia-portable-note"><strong>Path can really move backward.</strong> A forward &rarr; backward &rarr; forward route reads earlier IR samples again. Backtracking adds source-time travel, so the resulting response can become longer than a straight path.</div></article><div class="ia-mode-list"><article class="ia-mode-card"><b>Draw</b><h3>Author the A&rarr;B movement</h3><p>Paint a custom transition instead of choosing only Linear, Exp, Log or S-Curve.</p></article><article class="ia-mode-card"><b>Free endpoints</b><h3>Start and finish anywhere</h3><p>Free Draw can create shapes such as A &rarr; B &rarr; A or begin and end at custom blends.</p></article><article class="ia-mode-card"><b>Precision</b><h3>Undo, Smooth and Nodes</h3><p>Clean up hand-drawn shapes with editable precision nodes while preserving smooth interpolation.</p></article><article class="ia-mode-card"><b>Mode-aware Bake</b><h3>The movement becomes the file</h3><p>Draw and Path are rendered through the prepared IR path, so the construction you preview is what Bake writes into the reusable WAV.</p></article></div></div><div class="ia-actions"><a class="ia-btn ia-btn-primary" href="/docs/impulse-anvil/sections/curves-path/">Explore Draw &amp; Path</a><a class="ia-btn" href="/docs/impulse-anvil/workflows/path-time-routing/">Path workflow</a><a class="ia-btn ia-btn-dark" href="/docs/impulse-anvil/sections/morph/">Compare Morph modes</a></div></div></section>
...
<div class="ia-callout"><h3>Morph + Draw/Path + Lerp</h3><p>Choose six Morph modes, author the transition with Draw or Path, and bake mode-aware A→B movement into one prepared IR.</p></div>
...
<div class="ia-shell">
<div class="ia-section-head"><div class="ia-copy"><span class="ia-kicker">Pricing</span><h2>Try the workflow.<br/>Unlock the complete forge.</h2><p>The same VST3 download opens in demo mode. A license key unlocks the full morph engine, Draw/Path authoring, both Color rows, A→B Lerp and WAV baking.</p></div></div>
...
<ul class="ia-list"><li>Time Morph, Spectral, Spectral BandSwap and Spectral Zig Zag</li><li>Stereo Slot Swap and Mid/Side Boundary modes</li><li>Draw and Path authoring with millisecond Lerp timing</li><li>Mode-aware A→B Lerp and WAV export</li><li>Two Color rows per IR plus Texture Depth</li><li>Waveform editing, visual EQ, width and level preparation</li><li>2 seats per license</li></ul>
...
<details><summary>Can the exported IR be used outside Impulse Anvil?</summary><p>Yes. Full-version bakes are standard WAV impulse responses and can be loaded into compatible convolution reverbs, IR loaders, cabinet tools and compatible convolution software.</p></details>
...
`; } function buildForge(){
    ...
    // Docs & FAQ page
    function buildDocs(){ return `<div class="ia26" id="docs"><section class="ia-section ia-hero"><div class="ia-shell"><span class="ia-eyebrow"><span class="ia-pulse"></span> Impulse Anvil documentation</span><h1>The manual now lives in a <span class="ia-cyan">focused documentation system.</span></h1><p class="ia-lead">Open Quickstart, Draw & Path, Morph, workflows, Bake, reference and troubleshooting as separate permanent pages.</p><div class="ia-actions"><a class="ia-btn ia-btn-primary" href="/docs/impulse-anvil/">Open documentation</a><a class="ia-btn" href="/impulse-anvil.html">Product page</a><a class="ia-btn ia-btn-dark" href="/downloads.html">Downloads</a></div></div></section></div>`; }
    function buildAnvilDownloads(){
          return `
          <div class="ia26" id="downloads">
            <section class="ia-section ia-hero"><div class="ia-shell ia-hero-grid"><div><div class="ia-download-heading"><span class="ia-eyebrow"><span class="ia-pulse"></span> Download</span><span class="ia-current-version">Current v1.0.121</span></div><h1>Impulse Anvil <span class="ia-cyan">VST3 ZIP.</span></h1><p class="ia-lead">Download the Windows VST3 folder bundle, extract it, copy the complete <code>Impulse Anvil.vst3</code> folder into the VST3 directory, then rescan your DAW.</p><div class="ia-actions"><a class="ia-btn ia-btn-primary" href="${IMPULSE_ANVIL_DOWNLOAD}" target="_blank" rel="noopener">Download VST3</a><a class="ia-btn lemonsqueezy-button" href="${IMPULSE_ANVIL_BUY_URL}">Get license €29</a><a class="ia-btn ia-btn-dark" href="#download-install" onclick="return iaScrollTo('download-install');">Install guide</a><a class="ia-btn ia-btn-dark" href="/docs/impulse-anvil/">Documentation</a><a class="ia-btn ia-btn-dark" href="${DISCORD_INVITE_URL}" target="_blank" rel="noopener" aria-label="Join the Impulse Anvil Discord for setup help, free themes, free IRs, updates and videos">${DISCORD_ICON}Discord support</a></div></div><div class="ia-product-card"><div class="ia-product-window"><img src="assets/impulse-anvil-plugin-ui.png" alt="Impulse Anvil plugin interface" loading="lazy"></div></div></div></section>
...
<details open><summary>v1.0.121 &mdash; Draw, Path &amp; IR Authoring Update</summary><div class="ia-changelog-body"><p><span class="ia-version-pill">Current release</span></p><p>This update substantially expands Impulse Anvil&rsquo;s IR design system while keeping the existing A/B workflow.</p><h3>Draw Morph Curves</h3><ul class="ia-list"><li>Added <strong>Draw</strong> to the Morph Curve selector so A&rarr;B movement can be painted directly instead of using only Linear, Exp, Log or S-Curve.</li><li>Draw state is preserved with DAW projects and Impulse Anvil presets; the authored movement is included when the prepared IR is baked.</li><li>Preview and Bake use the same Draw construction.</li></ul><h3>Path &mdash; draw through source-time</h3><ul class="ia-list"><li>Added <strong>Path</strong>, which controls both A/B blend and travel through source-time.</li><li>Rightward movement reads forward, leftward movement reads earlier IR material again, upward moves toward B and downward moves toward A.</li><li>Backtracking is preserved in playback order and can make the rendered IR longer than a straight path.</li><li>Long continuous drawings are simplified internally while preserving direction, A/B changes, playback order and endpoints.</li></ul><h3>Draw tools</h3><ul class="ia-list"><li><strong>U</strong> &mdash; Undo.</li><li><strong>S</strong> &mdash; Smooth.</li><li><strong>N</strong> &mdash; Precision Nodes for editable control points.</li><li><strong>F</strong> &mdash; Free endpoints, allowing curves such as A &rarr; B &rarr; A or custom start/end blends.</li><li><strong>R</strong> &mdash; Reset.</li></ul><h3>Lerp timing reworked</h3><ul class="ia-list"><li>Lerp timing is now shown directly in milliseconds through <strong>Lerp Start</strong> and <strong>Lerp Time</strong>.</li><li>The old hidden minimum/start behavior has been removed.</li><li>The available range follows the actual prepared A/B duration, including Time and Color/Texture length changes.</li></ul><h3>A/B waveforms in the curve display</h3><ul class="ia-list"><li>The Morph curve display now shows the A and B waveforms as subtle visual landmarks.</li><li>This makes differing IR lengths, transients, gaps, tails and other events easier to identify while drawing Draw or Path movement.</li></ul><h3>True 0 ms Fade</h3><ul class="ia-list"><li>Fade In = 0 ms now means no authored fade at that endpoint.</li><li>This improves tiny IR slices, transient design, clicks, impulses and micro-texture work.</li></ul><h3>Color Offset extended</h3><ul class="ia-list"><li>Color Offset now reaches up to <strong>500 ms</strong>.</li><li>Small values remain useful for comb/phase coloration while larger offsets can create reflections, echoes, layered attacks and unusual spatial structures.</li><li>Randomization remains biased toward shorter values rather than constantly choosing extremes.</li></ul><h3>Theme Schema 2</h3><ul class="ia-list"><li>Themes can optionally style A, B, Morph, EQ, Edit and Out individually.</li><li>Each section can override panel, border, accent, highlight, knob and text colors while inheriting the global theme when no override is supplied.</li><li>Schema 1 themes remain compatible.</li><li>Customize Current Theme and Theme Export now support the Schema 2 panel structure.</li></ul><h3>Dialogs, browsing and tooltips</h3><ul class="ia-list"><li>Updated Bake and Theme Export confirmations can open the generated file location directly.</li><li>Load WAV now prefers the last browsed folder, then the Custom IR Folder, bundled Factory IRs and compatible legacy locations.</li><li>The Baked IRs folder remains in Documents so generated user files stay outside the plugin bundle.</li><li>Expanded tooltips across Time, Gain, Color, Morph/Lerp, Edit, EQ, Width, levels, Limiter and Bake controls.</li></ul><h3>Preview / Bake consistency</h3><ul class="ia-list"><li>Draw and Path use the same prepared rendering path for Preview and Bake, so the construction heard while designing is the construction exported.</li></ul><h3>Compatibility</h3><ul class="ia-list"><li>Windows 10/11, 64-bit VST3.</li><li>Existing presets and projects remain supported.</li><li>Old Power curve states migrate safely to the equivalent Exp behavior.</li><li>Existing Schema 1 themes remain supported.</li></ul><div class="ia-actions"><a class="ia-btn ia-btn-small ia-btn-primary" href="/docs/impulse-anvil/sections/curves-path/">Draw &amp; Path documentation</a><a class="ia-btn ia-btn-small" href="https://github.com/FreQtik/freqtikWebsite.github.io/releases/tag/v1.0.121" rel="noopener" target="_blank">GitHub release</a></div></div></details>
              <details><summary>v1.0.120 &mdash; JUCE 8 Maintenance Build</summary><div class="ia-changelog-body"><p>This maintenance release refreshes the Windows commercial build after migration to JUCE 8.0.13.</p><h3>Updated</h3><ul class="ia-list"><li>Rebuilt the Windows 64-bit VST3 with JUCE 8.0.13.</li><li>Updated the bundled license notices for the current release toolchain.</li><li>Corrected the license activation dialog layout after the JUCE migration.</li></ul><h3>Compatibility</h3><ul class="ia-list"><li>No intentional changes were made to the DSP workflow, parameter IDs, project-state format, factory IR library, license entitlement or demo restrictions.</li><li>Windows 10/11, 64-bit VST3.</li><li>Replace the complete existing <code>Impulse Anvil.vst3</code> folder when updating.</li></ul></div></details>
