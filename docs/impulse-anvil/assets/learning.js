...
(() => {
  const TERM_DEFS = {
    "impulse-response": "An impulse response (IR) is a short audio response that describes how a space, object, speaker, resonator or other system changes sound. Impulse Anvil can also use compatible recordings creatively as IR material.",
    "convolution": "Convolution applies an impulse response to incoming audio, making that audio inherit the response's filtering, resonance, reflections or spatial character.",
    "spectrum": "Spectrum means how a sound's energy is distributed across low, middle and high frequencies.",
    "phase": "Phase describes timing relationships inside a waveform, including how frequency components line up with one another.",
    "residual": "A residual is what remains after an estimated component has been removed from another response.",
    "mid-side": "Mid/Side represents stereo as center information (Mid) and left-right difference or width information (Side).",
    "source-time": "Source-time means which point in the prepared A/B material is being read. Moving backward in source-time revisits earlier material."
  };
  ...
  function render() {
    ...
    if (progressText) progressText.textContent = `${done} / ${quests.length} quests complete`;
    if (progressBar) progressBar.style.width = `${quests.length ? (done / quests.length) * 100 : 0}%`;
    if (showAllButton) showAllButton.textContent = state.showAll ? "Follow unlock order" : "Show all quests";
    ...
    quests.forEach((quest, index) => {
      ...
      if (button) {
        ...
        button.textContent = complete ? "Completed" : (id === "09" ? "Complete learning track" : "Mark complete");
      }
      if (status) status.textContent = complete ? "Completed" : (unlocked ? "Ready" : "Locked");
    });
  ...
  if (resetButton) {
    resetButton.addEventListener("click", () => {
      if (!window.confirm("Reset the local Impulse Anvil learning history on this browser?")) return;
