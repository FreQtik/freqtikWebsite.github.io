...
console.log("Impulse Anvil v1.0.122 - Optional Bake Semantic Sync FIX10");
...
try {
  ...
  if (process.env.FREQTIK_PATCH_TEST_MODE !== "1") {
    ...
    const gitCheck = cp.spawnSync("git", ["diff", "--check", "--", ...rels], { cwd: repo, encoding: "utf8", shell: process.platform === "win32" });
    if (gitCheck.error == null && gitCheck.status !== 0) fail("git diff --check failed:\n" + (gitCheck.stderr || gitCheck.stdout));
  }
} catch (err) {
  for (const [rel, buf] of originals) fs.writeFileSync(path.join(repo, rel), buf);
  console.error("\nPost-write validation failed. FIX10 restored the original files.");
  throw err;
}

console.log("\nChanged files:");
if (!changed.length) console.log("  none - FIX10 is already applied.");
